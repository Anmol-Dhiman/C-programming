...
void matrix_multiplication(int r1, int c1, int r2, int c2, int matrix1[10][10], int matrix2[10][10], int matrixFinal[10][10])
{
    ...
    // now we will multiply two matrices
    for (int i = 0; i < r1; i++)   // r1 = 2, c1 = 2
    {
        for (int j = 0; j < c1; j++)
        {
            for (int k = 0; k < c2; k++)
...
void Orthogonal(int matrix[10][10], int row1, int col1)
{
    ...
    for (int i = 0; i < row1; i++)
    {
        for (int j = 0; j < col1; j++)
        {
            ...
            {
                matrixFinal[i][j] += matrix[i][k] * transA[k][j]; // check with an example of 2 x 2
            }
        }
    }

    // now we got the product of A*A(transpose) lets check if its identity.
    // final matrix order will be row1 x row1
    int c1 = 0;
    for (int i = 0; i < row1; i++)
    {
        for (int j = 0; j < row1; j++)
...
void determinant(int a[10][10], int n)
{
    ...
    // only possible for 2x2 and 3x3 matrix
    if (n == 2)
    {
        determine = a[0][0] * a[1][1] - a[1][0] * a[0][1];
...
int main()
{
    // put this inside switch case of matrix multiplication
    int row1, row2, col1, col2, order;
    int arr1[10][10], arr2[10][10], arrFinal[10][10];
    char options;

    // option chosen should be an integer
    printf("Please choose the operation you want to perform:\nPress the corresponding option\n1.) Addition of two matrices\n2.) Subtraction of two matrices\n3.) Matrix Multilplication\n"); // continue
    printf("4.) Transpose of matrix\n5.) Orthogonality Checker\n6.) Determinant of matrix (2x2 and 3x3 only)\nOr press 'q' to quit\n");
    ...
    else if (options == '4' || options == '5' || options == '6')
    {
    ...
    case 'q':
        ...
        break;
    case '1':
        printf("Enter row1 and col1 dimensions: ");
        scanf("%d %d", &row1, &col1);

        printf("Enter row2 and col2 dimensions: ");
        scanf("%d %d", &row2, &col2);
        ...
        for (int i = 0; i < row1; i++)
        {
            ...
            {
                printf("Enter the %d,%d element of the matrix1 ", i, j);
                scanf("%d", &arr1[i][j]);
            }
        }
        for (int i = 0; i < row2; i++)
        {
            ...
            {
                printf("Enter the %d,%d element of the matrix2 ", i, j);
                scanf("%d", &arr2[i][j]);
            }
        }
        ...
        addition_of_two(row1, col1, arr1, arr2, arrFinal);
        break;
    case '2':
        printf("Enter row1 and col1 dimensions: ");
        scanf("%d %d", &row1, &col1);
    ...
    case '6':
        ...
        if (order == 1 || order == 2)
        {
            ...
            determinant(arr1, order);
            break;
        }
        else
        {
            printf("Please enter a matrix of order 2 or 3....\n Quitting the program");
            break;
        }
